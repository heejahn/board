        ...
        function showCommentList(){
            $.ajax({
                ...
                success : function(result){
                    let html = '';
                    html = '<table class="table table-sm table-borderless" id="commentTable"><tbody>';

                    if(result.length > 0) {
                        $(result).each(function() {
                            html += '<tr><td>'+ this.contents +'</td>' +
                                    '<td style="color: #95999c">' + this.commentRegDt.year + "/" + this.commentRegDt.monthValue + "/" + this.commentRegDt.dayOfMonth + '</td>' +
                                    '<td><a href=# id="deleteComment" onclick="deleteComment('+ this.cid +')">삭제</a></td></tr>';
                        });

                    } else {
                        html += '<td style="color: #95999c">등록된 댓글이 없습니다.</td>';
                    }
                    html += '</tbody></table>';

                    $('#showCommentList').append(html);
                }
            });
        }
